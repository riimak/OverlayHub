...
export default function ScoreboardPage({
  ...
  // Default colors (can still be overridden by query params)
...
(function(){
  ...
  async function tick(){
    try {
      ...
      // Apply theme vars from settings (if set)
      const leftColor = normalizeColor(settings.leftColor);
      const rightColor = normalizeColor(settings.rightColor);
      if (leftColor) setVar('--left', leftColor);
      if (rightColor) setVar('--right', rightColor);
      ...
      // Extract players
      const p1 = data.match.player1;
      const p2 = data.match.player2;
      ...
      if (event && typeof event.at === 'number' && event.at > lastEventAt) {
        ...
        if (event.type === 'slide') {
          el('box').classList.remove('slide');
          void el('box').offsetWidth;
          el('box').classList.add('slide');
          setTimeout(() => el('box').classList.remove('slide'), 900);
        }
      }
