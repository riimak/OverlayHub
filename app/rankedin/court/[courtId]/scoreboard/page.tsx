...
export default function ScoreboardPage({
  ...
  // Simple theme knobs (optional)
  const font = String(first(searchParams?.font, "Inter"));
  const cLeft = String(first(searchParams?.left, "#0b3aa6"));
  const cRight = String(first(searchParams?.right, "#c66a08"));

  // Logo controls
  const logoOpacity = Number(first(searchParams?.logoOpacity, "0.9"));
  const logoScale = Number(first(searchParams?.logoScale, "1")); // 1 = default

  const safeLogoOpacity = Number.isFinite(logoOpacity) ? Math.min(1, Math.max(0, logoOpacity)) : 0.9;
  const safeLogoScale = Number.isFinite(logoScale) ? Math.min(2, Math.max(0.4, logoScale)) : 1;
    ...
    <html>
      <head>
        ...
        <style>{`
          ...
          html, body { margin:0; background: transparent; font-family: ${JSON.stringify(font)}, Inter, Arial, sans-serif; }
          ...
          /* Logo centered underneath everything */
          .logoWrap{
            width: 900px;
            display:flex;
            justify-content:center;
            margin-top: 10px;
            opacity: ${safeLogoOpacity};
            transform: scale(${safeLogoScale});
...
(function(){
  ...
  async function tick(){
    try {
      ...
      el('status').textContent = (data.match.status || 'LIVE').toUpperCase();
